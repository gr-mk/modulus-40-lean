#!/usr/bin/env python3
"""Backward-slice the support-uniqueness LRAT certificate into pure RUP data."""
import argparse
import json
from pathlib import Path
from export_rup_certificate import read_cnf, rup
ROOT=Path(__file__).resolve().parents[1]

def main():
    parser=argparse.ArgumentParser()
    parser.add_argument('--source',type=Path,default=ROOT/'tmp/circuit/Support')
    parser.add_argument('--out',type=Path,default=ROOT/'data/certificates/support-rup.json')
    args=parser.parse_args()
    initial=read_cnf(args.source.with_suffix('.cnf'))
    steps={}
    with args.source.with_suffix('.lrat').open() as stream:
        for line in stream:
            ss=line.split()
